...
class Tracker:
    ...
    def _get_variable_value(self, network, name: str, view: str) -> Any:
        """
        ...
        try:
            return network.get_val(name, view)
        except (AttributeError, TypeError) as e:
            warnings.warn(f"Error retrieving {name} ({view}): {e}")
            return None

    def track(self, network, time: Union[int, float]) -> None:
        """
        ...
        if self.track_counter % self.track_step == 0:
            for name, view in self.variables:
                key = f"{name}_{view}"
                val = self._get_variable_value(network, name, view)
...
class TrackerManager:
    def __init__(self, num_seeds, *tracker_args, **tracker_kwargs):
        self.trackers = [
            Tracker(*tracker_args, **tracker_kwargs) for _ in range(num_seeds)
        ]

    def track(self, time, seed_idx):
        self.trackers[seed_idx].track(time)

    def store(self, value_name, val, seed_idx):
        self.trackers[seed_idx].store(value_name, val)

    def get_all(self, key):
        return [tracker[key] for tracker in self.trackers]
